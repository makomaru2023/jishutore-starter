 ...
 *   - 有期契約のときは contractPeriod・contractRenewal を必ず書く。
 *     更新上限（通算年数・更新回数）を設けているときは contractRenewalLimit も書く
 *   - 所定労働時間を超える労働があるときは overtimeWork に書く（workHours に混ぜない）
 *   - 固定残業代（みなし残業代）があるときは fixedOvertimeAllowance に内訳を書く。
 *     金額・含まれる時間数・超過分は別途支給、の3点が揃っていないと賃金の誤解を生む
 *   - 「なし」と書く必要はない。書かなかった項目は詳細ページに出ない。
 *     制度が無いのに既定値を入れると、条件を断定したことになる
...
export const jobs: Job[] = [
     ...
     */
    {
        ...
        workHours: "8:30〜17:30（休憩60分／所定労働時間8時間）",
        overtimeWork: "あり（月平均5時間程度）。時間外手当は全額支給します。",
        ...
        applicationMethod:
            "公式採用ページの応募フォームからお申し込みください。書類選考のうえ、面接日程をご連絡します。",
