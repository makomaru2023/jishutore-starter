...
export interface Job {
    ...
    salaryNote?: string;
    /**
     * 固定残業代（みなし残業代）がある場合の内訳。
     * 固定残業代の額、その額に含まれる時間外労働の時間数、
     * 超過分を別途支払うことの3点を書く。
     * ★固定残業代があるのに書かないと、賃金の誤解を生じさせる表示になりうる。
     *   制度が無いときは書かない（「なし」と書く必要はない）。
     */
    ...
    workHours: string;
    /**
     * 所定労働時間を超える労働（時間外労働）の有無と見込み。
     * 職業安定法の明示事項。workHours の文中に混ぜず、行を分けて出す。
     * ★workplace.overtime（残業の実態・任意の職場情報）とは別物。
     *   こちらは募集条件そのもの、あちらは「実際どのくらいか」の開示。
     */
    overtimeWork?: string;
    /** 休日・休暇 */
    holidays: string;
    ...
    /**
     * 有期労働契約の更新上限（通算契約期間または更新回数の上限）。
     * 2024年4月1日から明示事項。上限を設けている場合のみ書く。
     * ★上限が無いのに「上限なし」と書かせない。書かなければ行ごと出ない。
     */
    contractRenewalLimit?: string;
    /** 加入保険（労災・雇用・健康・厚生年金） */
    insurance?: string;
    ...
    contact?: string;
    /**
     * 応募方法（例：「公式採用ページの応募フォームから」「電話連絡のうえ履歴書を郵送」）。
     * ★応募は施設側で完結するので、求職者が最初に何をすればよいかを書く。
     *   書かなかった場合は officialRecruitUrl へのボタンだけが出る。
     */
