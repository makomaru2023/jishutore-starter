...
export const metadata: Metadata = {
    ...
    description: `${PLUS_SLIDE_COUNT}点の運動スライド（毎月追加中）から必要なページを選び、PowerPointで編集・組み替えできる月額サービスです。訪問リハ・通所リハ・老健・訪問看護からのリハの診療・介護報酬チェックも追加。7月登録は永続月額500円（8月〜680円、以降は素材点数に応じて改定）。既存会員は据え置き。`,
    alternates: {
...
const features = [
    ...
    {
        title: "診療・介護報酬も確認できる",
        body: "訪問リハ・通所リハ・老健・訪問看護からのリハの単位数・記録・根拠資料を確認できるツールを追加しました。",
    },
] as const;
...
const faqs: { q: string; a: string }[] = [
    ...
    },
    {
        ...
        a: "訪問リハビリテーション・通所リハビリテーション・介護老人保健施設（老健）・訪問看護からのリハ（理学療法士等訪問）の単位数や点数、算定要件、記録に残すことを、厚生労働省の根拠資料リンクつきで確認できます。",
    },
    {
        q: "月の途中で登録すると損しませんか？",
                        ...
                        <div className="flex flex-col gap-4 rounded-lg border border-blue-100 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
                            <div className="min-w-0">
                                ...
                                <p className="mt-2 text-sm leading-6 text-slate-600">
                                    訪問リハ・通所リハ・老健・訪問看護からのリハの単位数・点数・記録に残すことを確認できます。
                                </p>
                            </div>
                            <Link
                                href="/plus/fee-check/"
