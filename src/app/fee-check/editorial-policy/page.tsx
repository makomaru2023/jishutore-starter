...
export const metadata: Metadata = {
    ...
    description:
        "診療・介護報酬チェックの編集方針を公開しています。作業療法士である運営者が、厚生労働省の告示・通知・疑義解釈などの一次資料を基準に、単位数・算定要件・記録・自己点検ポイントを確認・更新する方法、掲載範囲と注意事項、対象年度・根拠資料リンク・最終確認日の扱いを説明します。",
...
const sections = [
    ...
    },
    {
        title: "対象年度と最終確認日の表示",
        body: [
            "各分野のページと項目ページには、どの年度の制度を対象としているか（対象年度）を表示しています。介護保険と医療保険で改定の年がずれるため、保険ごとに分けて表示しています。分野のページには、期中改定や次回改定の予定を含む詳しい版の情報も掲載しています。",
            "各項目には、その項目について一次資料を確認した最終確認日を表示しています。まだ確認できていない項目には日付を表示せず、「未確認」と表示します。確認していない項目に日付を入れることはしません。",
            "掲載している内容は、掲載時点および表示している最終確認日時点のものです。診療報酬・介護報酬は、改定、疑義解釈の発出、事務連絡、告示の訂正等により変更されることがあります。最終確認日から時間が経っている項目については、原本での確認をお願いします。",
        ],
    },
    {
        title: "更新方法",
        body: [
            "診療報酬・介護報酬の改定、疑義解釈、事務連絡、官報訂正を定期的に確認し、影響する項目を更新します。更新した項目には最終確認日を表示します。",
            "制度改定があった場合は、対象年度の表示を新しい年度に切り替えたうえで、影響する項目を順に確認していきます。切り替えが済んでいない項目は、最終確認日が改定前の日付のままになるため、表示を見れば見分けられます。",
